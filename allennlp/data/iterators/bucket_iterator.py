...
class BucketIterator(DataIterator):
    ...
    def _create_batches(self, instances: Iterable[Instance], shuffle: bool) -> Iterable[Batch]:
        #  传进来的instances是所有的数据
        # _memory_sized_lists作用: Breaks the dataset into "memory-sized" lists of instances
        for instance_list in self._memory_sized_lists(instances):
            # lazy时建议设置max_instances_in_memory,内存足够时尽可能多的instance,这样进行后续的排序才有意义
            # 在_sort_by_padding中会实现tokens的索引化
            instance_list = self._sort_by_padding(instance_list)

            batches = []
            excess: Deque[Instance] = deque()
            # lazy_groups_of函数将instance_list这批数据每次返回一个批次的数据
            for batch_instances in lazy_groups_of(iter(instance_list), self._batch_size):
                for possibly_smaller_batches in self._ensure_batch_is_sufficiently_small(batch_instances, excess):
                    if (self._skip_smaller_batches and len(possibly_smaller_batches) < self._batch_size):
                        continue
                    # Batch解释:包含Instance,和一些将Instance转为tensor的函数
                    # 每个Batch包含独立的数据,是一个实例,后面会在每个实例中将文本索引化
