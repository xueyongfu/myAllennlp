...
class PretrainedTransformerIndexer(TokenIndexer):
    ...
    def tokens_to_indices(self, tokens: List[Token], vocabulary: Vocabulary) -> IndexedTokenList:
        ...
        # mask长度与经过后处理的token_ids的长度可能不同.
        # 但是进过后处理的token_ids的长度与segment_concat_mask的长度是一样的
    ...
    def _postprocess_output(self, output: IndexedTokenList) -> IndexedTokenList:
        ...
        if self._max_length is not None:
            ...
            # 限制到最大长度
            output["token_ids"] = output["token_ids"][:self._max_length]
            output["type_ids"] = output["type_ids"][:self._max_length]
            output["segment_concat_mask"] = output["segment_concat_mask"][:self._max_length]
