...
class Trainer(TrainerBase):
    ...
    def batch_loss(self, batch: TensorDict, for_training: bool) -> torch.Tensor:
        ...
        output_dict = self._pytorch_model(**batch)   #**batch将batch当做函数参数传入
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        # self.iterator(self.train_data, num_epochs=1, shuffle=self.shuffle) 解释: self.iterator是BucketIterator类
        # class()表示调用类的__call__方法,该方法也是一个迭代器
        batch_generator = self.iterator(self.train_data, num_epochs=1, shuffle=self.shuffle)
        # common_util.lazy_groups_of()会生成一个迭代器, 即batch_group_generator是个迭代器
        batch_group_generator = common_util.lazy_groups_of(
            batch_generator, self._num_gradient_accumulation_steps
        )

        # 除以梯度积累数的批次是每次送入神经网络的每个批次的数量.
        # 模型总的效果还是超参数中的批次
        ...
        for batch_group in batch_group_generator_tqdm:
            ...
            for batch in batch_group:   #batch_group中的数量等于梯度累计数
